...
# Prediction
heart_test$prediction <- predict(mod1, newdata = heart_test, type = "response")
heart_train$prediction  <- predict(mod1, newdata = heart_train, type = "response")
...
library(rpart)
library(rpart.plot)
tree<-rpart(HeartDisease ~ Stroke + Diabetic + Asthma + KidneyDisease 
              + SkinCancer,method = "class",data =heart_train)
rpart.plot(tree)
```
