...
```{r}
lm2 <- lm(HeartDisease ~ Smoking,data = project)
xkablevif(lm2)
summary(lm2)
...
```{r}
lm3 <- lm(HeartDisease ~ Diabetic,data = project)
xkablevif(lm3)
summary(lm3)
```
...
```{r}
library(dyn)
result <- anova(lm1,lm2,lm3)
xkabledply(result, title = "ANOVA comparison between the three models")
```
