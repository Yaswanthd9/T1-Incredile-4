...
AccuracyCutoffInfo <- function( train, test, predict, actual )
{
  ...
  cutoff <- seq( .1, .9, by = .05 )  # EL

	accuracy <- lapply( cutoff, function(c)
	{
		# use the confusionMatrix from the caret package

	  # Fix new Error: `data` and `reference` should be factors with the same levels.  # EL
	  cm_train <- confusionMatrix( factor( as.numeric( train[[predict]] > c ) ), factor( train[[actual]] ) )
	  cm_test  <- confusionMatrix( factor( as.numeric( test[[predict]]  > c ) ), factor( test[[actual]] )  )
