...
dresstest <- subset(heart, split == FALSE)
model <- glm (HD ~ GeneralHealth+PhysicalActivity, data = dresstrain, family = binomial)
summary(model)
predict <- predict(model, type = 'response')
table(dresstrain$HD, predict > 0.5)
#ROCR Curve
library(ROCR)
ROCRpred <- prediction(predict, dresstrain$HD)
ROCRperf <- performance(ROCRpred, 'tpr','fpr')
plot(ROCRperf, colorize = TRUE, text.adj = c(-0.2,1.7))
#plot glm
library(ggplot2)
ggplot(dresstrain, aes(x=GeneralHealth+PhysicalActivity, y=HD)) + geom_point() + 
...
1/(1+exp(-predict(model, newdata = newdata1))) # 1/(1+exp(-predict(admitLogit, newdata = newdata1, type="link")))
