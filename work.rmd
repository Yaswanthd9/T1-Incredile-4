...
dressheart <- subset(heart, split == TRUE)
dresstest <- subset(heart, split == FALSE)
model <- glm (HD ~ GeneralHealth+PhysicalActivity, data = dressheart, family = binomial)
summary(model)
predict <- predict(model, type = 'response')
table(dressheart$HD, predict > 0.5)
#ROCR Curve
library(ROCR)
ROCRpred <- prediction(predict, dressheart$HD)
