...
```{r, results='markup'}
act <- subset(heart,PhysicalActivity==1)
head(patient)
na_act <- subset(heart,PhysicalActivity==0)
```

```{r}
ggplot(patient, aes(x=PhysicalActivity)) + geom_bar(color="black",fill="darkblue")
dia_pa<-subset(act,PhysicalActivity==1)
nrow(dia_pa)
ggplot(na_act, aes(x=PhysicalActivity)) + geom_bar(color="black",fill="darkblue")+ scale_y_continuous(labels = scales::comma)
dia_pa1<-subset(na_act,PhysicalActivity==1)
nrow(dia_pa1)
```
...
```{r}
ggplot(heart,aes(x = PhysicalActivity, fill = HeartDisease)) + geom_bar(position = "fill") +ggtitle("HeartDisease vs PhysicalActivity")
```
...
```{r}
barplot(table(heart$PhysicalActivity, heart$HeartDisease), legend= rownames(table(heart$HeartDisease, heart$PhysicalActivity)), col =(c("orange", "blue")), xlab="PhysicalActivity", ylab="heartdisease",main = "Barplot for Heartdisease and PhysicalActivity")
```

```{r}
library(ggplot2)

ggplot(heart, aes(fill=HeartDisease, x=PhysicalActivity, y=HeartDisease)) + 
...
```{r}
heart$dummy_HeartDisease <- factor(heart$HeartDisease, labels=c("No", "Yes"))
ggplot(heart, aes(x=PhysicalActivity, fill=dummy_HeartDisease)) + geom_bar(position="dodge")

```

```{r}
heart$dummy_HeartDisease <- factor(heart$HeartDisease, labels=c("No", "Yes"))
ggplot(heart, aes(x=GenHealth, fill=dummy_HeartDisease)) + geom_bar(position="dodge")
```
...
```{r}
mod1 <- glm(HD ~ GenHealth+PhysicalActivity,data=train,family = "binomial")
summary(mod1)
```
...
library(pROC)
library(ROCR)
test_prob <- predict(mod1, newdata = test, type = "response")
train_prob <-predict(mod1, newdata = train, type = "response")
test_roc <- roc(test$HD ~ test_prob, plot = TRUE, print.auc = TRUE)
```

```{r}
fittedheartTrain=ifelse(train_prob<0.5,0,1 )
ConfMatTrain=table(train$HD,fittedheartTrain)
...
ConfMatTest=table(test$HD,fittedheartTest)
...
```{r}
barplot(table(total$HeartDisease, total$GenHealth), legend= rownames(table(total$HeartDisease, total$GenHealth)), col =(c("blue", "red")), xlab="GeneralHealth", ylab="heartdisease",main = "Barplot for Heartdisease and GeneralHealth")
```