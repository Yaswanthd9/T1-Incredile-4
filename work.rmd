...
```{r}
### Linear SVM
svm_linear <- train(HD ~GeneralHealth+Ethnicity, data = train, method = "svmLinear", trControl=trainControl(method = "repeatedcv", number = 10, repeats = 3), preProcess = c("center", "scale"), tuneLength = 10)
svm_linear
```
...
confusionMatrix(table(test_pred,test$HD))
```

```{r}
### griding for obtaining best C value
svm_grid<-train(HD ~GeneralHealth+Ethnicity, data = train, method = "svmLinear", trControl=trainControl("cv", number = 10), tuneGrid = expand.grid(C=seq(0,2,length=20)), preProcess = c("center", "scale"))
```
