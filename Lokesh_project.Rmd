...
=======
```{r}
predicted<- predict(mod1, heart_test, type = "response")

hist(predicted)
```
```{r}
pre_mod1 <- as.numeric(predicted > 0.50)

table(heart_test$HeartDisease, pre_mod1, dnn = c("Actual", "Predicted"))
```
```{r}
mean(heart_test$HeartDisease == pre_mod1)
```
...
prob=predict(mod1, type = "response" )
total$prob=prob
h <- roc(HeartDisease~prob, data=total)
auc(h) # area-under-curve prefer 0.8 or higher.
plot(h)
# unloadPkg("pROC")
```
