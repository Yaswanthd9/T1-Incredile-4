...
```
```{r}
heart$GenHealth <- ifelse(heart$GenHealth == 'Poor',0,
               ifelse(heart$GenHealth == 'Fair',1,
                 ifelse(heart$GenHealth == 'Good',2,
                        ifelse(heart$GenHealth=='Very good',3,4
                        ))))
```
...
```{r}
set.seed(100)
heart_train_rows <- sample(1:nrow(heartsub), 0.7*nrow(heartsub))
heart_train <- heartsub[heart_train_rows, ]
heart_test <- heartsub[-heart_train_rows, ]
```
...
fittedGenderTrain=ifelse(fittedSamplesTrain<0.5,"Yes","No" )
...
fittedGenderTest=ifelse(fittedSamplesTest<0.5,"Yes","No" )
