...
```{r}
lm1 <- lm(HeartDisease ~ AlcoholDrinking + Smoking + AlcoholDrinking:Smoking, data = heart)
...
```{r}
lm3 <- lm(Diabetic ~ Smoking + AlcoholDrinking + Smoking : AlcoholDrinking,data = heart)
...
```{r}
percent <- xtabs(~heart$AlcoholDrinking+ heart$Smoking + heart$HeartDisease)
...
model_lr2 <- glm(HeartDisease ~ Smoking + AlcoholDrinking + Diabetic, data = train , family = "binomial")
summary(model_lr2)
xkabledply(model_lr2, title = paste("Logistic Regression :", format(formula(model_lr2))))
```

```{r}
test$model_prob <- predict(model_lr2, test, type = "response")

test <- test %>% mutate(accurate = 1*(model_prob == HeartDisease))
