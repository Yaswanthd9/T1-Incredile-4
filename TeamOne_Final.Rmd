...
```{r, results = 'markup'}
Regression <- lm(HeartDisease~GeneralHealth+PhysicalActivity, data=heart)
BIC(Regression)
summary (Regression)
```
...
```{r}
Regression3 <- lm(HD~GeneralHealth+PhysicalActivity+Ethnicity, data=heart)
BIC(Regression3)
summary (Regression3)
```
...
# 
cm_info <- ConfusionMatrixInfo( data = test, predict = "prediction", actual = 'HeartDisease', cutoff = .49 )
#ggthemr("flat")
cm_info$plot
