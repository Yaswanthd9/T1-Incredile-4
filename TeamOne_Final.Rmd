...
*** We created another dataframe that had equal proportions of 0 & 1 HD and used that for our training set... 
The nukmbers not used for the training were used for the test set ## Fix language 
...
Looking at the anova chi-squared results, we can conclude that the logit model (HeartDisease~ Sex+ BMI + PhysicalHealth + MentalHealth +  AgeCategory) used is a good fit model for predicting the probability of getting heart disease. All the coefficient p-values in the model are also significant, except for the 25-39 age range, which is significant at the 10% level.

According to the model, the odds ratio for getting heart disease increases by 2.07 for a man compared to a woman. While an increase in BMI increases the odds for heart disease by 1.03. Mental health and Physical health also affect your chances of getting heart disease but are not the most salient risk factors; they could be used as early indicators. As expected, the odds ratios for age increases as an individual gets older, i.e. older individuals are more likely to suffer poor health that leads to heart disease.  
...
After obtaining the predicted values that an individual will have heart disease on both the training and testing sets, a double density plot was drawn to show those predictions.  
...
# Freq plot for Heart Disease
```{r}
library(ggplot2)
library(ggpubr)
theme_set(theme_pubr())
ggplot(heart, aes(HeartDisease),fill=HeartDisease) +
  geom_bar() +
  theme_pubclean()
