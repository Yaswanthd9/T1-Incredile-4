...
With each new addition of a variable, we see that the adjusted R-squared is increasing indicating that the model fit is improving. However, the fit statistic is still quite low. 

** Check VIF to select variables 
```{r}
ezids::xkablevif(reg_3)
```

From, the VIF values, VUF for sleep time is 11.3 so we will drop. Race will aslo be dropped since it is super unbalanced 


# Logistic Regression  

# Separate to  80-20 Train-test Sets 
```{r}
...
set.seed(101) 
sample <- sample.int(n = nrow(heart), size = floor(.75*nrow(heart)), replace = F)
train <- heart[sample, ]
test  <- heart[-sample, ]

# Logit Model 1
model_1<- glm(HeartDisease~ Sex+ BMI, data=train, family='binomial')
summary(model)
...
Since, the chi-squared p-value of 0 is less than 0.05, we can conclude that the logit model with age and pclass as predictors is good fit model for predicting the probability of surviving. 

```{r}

# Logit Model 2 
model_2<- glm(HeartDisease~ Sex+ BMI + PhysicalHealth + MentalHealth +  AgeCategory, data=train, family='binomial')
summary(model_2)

1-pchisq(186906 - 157118, df=(319794-319772))

# Exponential Coefficients 
expcoeff = exp(coef(model_2))
xkabledply( as.table(expcoeff), title = "Exponential of Coefficients in Logit Reg" )
```

# testing using model_2
```{r}

test$model_prob <- predict(model_2, test, type = "response")

test <- test  %>% mutate(model_pred = 1*(model_prob > .53) + 0, 
                                  heart_disease = 1*(HeartDisease == 1) + 0)

#How do I show the predictions??? 

# Model Accuracy

test <- test %>% mutate(accurate = 1*(model_pred == heart_disease))
...
xkabledply( confusion_matrix(test), title = "Confusion matrix from Logit Model" )
unloadPkg("regclass")
```
